...
module.exports = () ->
  send: ((req, res) ->
    Cache.get "user:#{req.session.user_id}", (cache_user) ->
      if cache_user
        params =
          user_id: cache_user[0].user_id
          ins_id: cache_user[0].ins_id
  ...
  filter_spaces: ((req, res) ->
    Cache.get "user:#{req.session.user_id}", (cache_user) ->
      if cache_user
        if req.query?.type_spa and req.query?.time_interval and req.query?.capacity and req.query?.day and req.query?.month and cache_user[0].ins_id
          params =
  ...
  all_time_interval: ((req, res) =>
    Cache.get "user:#{req.session.user_id}", (cache_user) ->
      if cache_user
        if cache_user[0].ins_id
          Request.connect (client) =>
            Request.get_all_time_interval client, cache_user[0].ins_id, (resp) ->
              if resp.status is 'OK' and resp.data?
                res.send status: 'OK', data: resp.data
              else
  ...
  all_type_space: ((req, res) =>
    Cache.get "user:#{req.session.user_id}", (cache_user) ->
      if cache_user
        if cache_user[0].ins_id
          Request.connect (client) =>
            Request.get_all_type_space client, cache_user[0].ins_id, (resp) ->
              if resp.status is 'OK' and resp.data?
                res.send status: 'OK', data: resp.data
              else
                res.send status: 'ERROR', data: resp.data
  ...
  all_type_request: ((req, res) =>
    Cache.get "user:#{req.session.user_id}", (cache_user) ->
      if cache_user
        if cache_user[0].ins_id
          Request.connect (client) =>
            Request.get_all_type_request client, cache_user[0].ins_id, (resp) ->
              if resp.status is 'OK' and resp.data?
                res.send status: 'OK', data: resp.data
              else
                res.send status: 'ERROR', data: resp.data
